...
开发者预览期，宿主会同时存在多条线，例如 `0.1.2-rc.1`、`0.1.5-alpha.1`、`0.1.5-rc.1`、`0.1.6-alpha.2`。只钉一条线的组合，在用户从另一条线安装的那一刻就坏。

同时接受全部窗口的写法（每个预发布 tuple 一条子句；`-0` 下界覆盖 0.1.6 tuple 的全部预发布，绝不写成裸 `>=0.1.6` 形态）：
